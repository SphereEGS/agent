...
MAX_RECOGNITION_WORKERS = 4  # Number of parallel LPR threads
...
class Tracker:
    ...
    def track_and_capture(self) -> Generator[tuple[Any, List[tuple[int, NDArray[Any]]]], None, None]:
        ...
        for result in results:
                    ...
                    if in_roi:
                        ...
                        if vehicle["status"] == "pending" and vehicle["attempts"] < self.max_attempts:
                            ...
                            if self.recognition_queue.qsize() < RECOGNITION_QUEUE_MAX_SIZE:
                                future = self.executor.submit(self._recognize_plate_async, track_id, car_crop, vehicle["attempts"] + 1)
                                vehicle["attempts"] += 1
                            else:
            ...
            try:
                while True:
                    track_id, license_text, total_time = self.recognition_queue.get_nowait()
                        ...
                        if license_text:
                            ...
                            if self.backend_sync.is_authorized(license_text):
                                ...
                                if vehicle["first_frame"] is not None:
                                    self.backend_sync.log_to_backend(self.gate_type, license_text, True, vehicle["first_frame"], track_id)
    ...
    def _handle_unauthorized(self, track_id: int, frame: NDArray[Any] | None) -> None:
        ...
        if readings and any(reading is not None for reading in readings):
            ...
            logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} unauthorized after {vehicle['attempts']} attempts - No valid plate readings detected")
