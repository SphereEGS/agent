...
class Tracker:
    def __init__(self, gate_type: str, camera_url: str, roi_points: List[List[int]], model_path: str = "yolo11n.pt") -> None:
        ...
        if os.path.exists(tensorrt_path):
            ...
            logger.info(f"Gate {config.gate} ({gate_type}): Exporting YOLO model to TensorRT for Jetson Nano GPU...")
            try:
                self.model = YOLO(model_path, task="detect")
                self.model.export(format="engine", device="cuda", half=True)
                self.model = YOLO(tensorrt_path, task="detect")
                logger.info(f"Gate {config.gate} ({gate_type}): TensorRT YOLO model exported and loaded successfully")
            except Exception as e:
                logger.warning(f"Gate {config.gate} ({gate_type}): Failed to export to TensorRT: {e}. Falling back to PyTorch model.")
                self.model = YOLO(model_path, task="detect")
    ...
    def draw_roi(self) -> List[List[int]]:
        ...
        if not cap.isOpened():
            raise ValueError(f"Gate {config.gate} ({self.gate_type}): Could not open video stream")
        ret, frame = cap.read()
        if not ret:
            cap.release()
                ...
                for box in result.boxes:
                    ...
                    if in_roi:
                        ...
                        if vehicle["status"] == "pending":
                            ...
                            vehicle["attempts"] += 1
                            logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} - Recognition attempt {vehicle['attempts']}/{self.max_attempts}")
                            license_text = self.lpr.recognize_plate(car_crop) if car_crop.size > 0 else None

                            if license_text:
                                vehicle["readings"].append(license_text)
                                logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} - Plate reading: {license_text}")
                                ...
                                if self.backend_sync.is_authorized(license_text):
                                    ...
                                    self.backend_sync.log_to_backend(self.gate_type, license_text, True, original_frame, track_id)
                                elif vehicle["attempts"] >= self.max_attempts:
                                    self._handle_unauthorized(track_id, original_frame)
                            elif vehicle["attempts"] >= self.max_attempts:
                                logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} - No plate detected after {self.max_attempts} attempts")
                                vehicle["status"] = "no_plate"
                                vehicle["plate"] = "No plate found"
                                self.backend_sync.log_to_backend(self.gate_type, "No plate found", False, original_frame, track_id)

                        elif vehicle["status"] in ["authorized", "unauthorized", "no_plate"]:
                            status = "authorized" if vehicle["authorized"] else "unauthorized"
                            logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} with plate {vehicle['plate']} has been logged and is {status}")
                            if vehicle["authorized"]:
                                logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} still in ROI - Gate remains open")
                        ...
                        if vehicle["authorized"]:
                            ...
                            logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} with plate {vehicle['plate']} left ROI - No action taken")
            ...
            for track_id in list(self.tracked_vehicles.keys()):
                if track_id not in current_track_ids:
                    ...
                    if vehicle["authorized"]:
                        ...
                        logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} with plate {vehicle['plate']} no longer detected - No action taken")
                    del self.tracked_vehicles[track_id]
    ...
    def _handle_unauthorized(self, track_id: int, frame: np.ndarray):
        ...
        if readings:
            ...
            vehicle["plate"] = plate
            self.backend_sync.log_to_backend(self.gate_type, plate, False, frame, track_id)
        else:
            logger.info(f"Gate {config.gate} ({self.gate_type}): Vehicle {track_id} unauthorized after {self.max_attempts} attempts - No plate readings detected")
            vehicle["status"] = "no_plate"
            vehicle["plate"] = "No plate found"
            self.backend_sync.log_to_backend(self.gate_type, "No plate found", False, frame, track_id)